...
export default async function TeacherPage() {
  ...
  const myAssignmentsRaw = session?.uid
  ...
  const myAssignments: AssignmentRow[] = myAssignmentsRaw as unknown as AssignmentRow[];
    ...
    <div className="row" style={{ alignItems: "flex-start" }}>
      ...
      <div className="card" style={{ flex: 1, minWidth: 340 }}>
        ...
        <div style={{ display: "grid", gap: 10 }}>
          {myAssignments.map((a: AssignmentRow) => (
            <div key={a.id} className="card">
              <b>{a.title}</b>
              <div className="small">{a.description}</div>
              <div className="row" style={{ marginTop: 8 }}>
                <a className="button" href={`/sim?assignment=${a.id}`}>
                  Open
                </a>
                <a className="button" href={`/teacher/submissions?assignment=${a.id}`}>
                  Submissions
                </a>
              </div>
            </div>
          ...
          {myAssignments.length === 0 && (
            <p className="small">No assignments created yet.</p>
          )}
        </div>
      </div>
    </div>
